...
LOG_FILE="${BRMMEDIA_SMOKE_LOG_DIR:-/srv/brmmedia/logs}/smoke-music.log"

mkdir -p "$(dirname "$LOG_FILE")"
exec >>"$LOG_FILE" 2>&1
echo "===== smoke-music start $(date -Is) ====="
cd "$BACKEND_DIR"
set -a
...
exec "$BACKEND_DIR/.venv/bin/python" "$SCRIPT_ROOT/smoke_music.py"
