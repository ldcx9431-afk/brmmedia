...
command -v uv >/dev/null || { echo "[ERROR] uv is required to install the isolated Python 3.11 runtime" >&2; exit 1; }
command -v git >/dev/null || { echo "[ERROR] git is required" >&2; exit 1; }
command -v nvidia-smi >/dev/null || { echo "[ERROR] NVIDIA driver is unavailable" >&2; exit 1; }

gpu="$(nvidia-smi --query-gpu=name,compute_cap,memory.total,driver_version --format=csv,noheader,nounits | sed -n '1p')"
...
(
  cd "$SOURCE_ROOT"
  UV_PROJECT_ENVIRONMENT="$VENV_ROOT" uv sync --locked
)
